...
% Calculate the uncram width, binary point, and arithmetic type
% which will be different depending on whether inputs are complex
if strcmp(complex_in, 'on'),
    uncram_slice_width = 2*input_bit_width;
    uncram_bin_pt = 0;
    uncram_arith_type = '0';
else
    uncram_slice_width = input_bit_width;
    uncram_bin_pt = input_bin_pt;
    uncram_arith_type = '1';
end
...
for i=0:2^floor(vector_len-samples_out)-1,
    ...
    reuse_block(blk, ['uncram', num2str(i)], 'gavrt_library/uncram',...
        ...
        'Bin_Pt', num2str(uncram_bin_pt),...
        'Arith_Type', uncram_arith_type);
    ...
    for j=0:2^samples_out-1,
        ...
        if strcmp(complex_in, 'off'),
            acc_type = 'real';
            acc_name = [acc_type, '_acc', num2str(i*2^samples_out+j)];
            reuse_block(blk, acc_name,...
                'cfa_library/simple_acc',...
                ...
                'Bit_Width', num2str(output_bit_width),...
            ...
            acc_name = [acc_type, '_acc', num2str(i*2^samples_out+j)];
            reuse_block(blk, acc_name,...
                'cfa_library/complex_acc',...
                'Position', [x0+(16+j*2)*width y0+10*j+(i+3)*port_ysep,...
                x0+(17+j*2)*width y0+10*j+(i+3)*port_ysep+height],...
                'Samples', num2str(floor(vector_len-samples_out)),...
                'Input_Bit_Width', num2str(input_bit_width),...
                'Input_Bin_Pt', num2str(input_bin_pt),...
                'Bit_Width', num2str(output_bit_width),...
                'Bin_Pt', num2str(output_bin_pt),...
                'Quantization', quantization,...
                'Overflow', overflow,...
...
% Calculate the last uncram width, binary point, and arithmetic type
if strcmp(complex_in, 'on'),
    last_uncram_slice_width = 2*output_bit_width;
    last_uncram_bin_pt = 0;
    last_uncram_arith_type = '0';
    convert_arith_type = 'Unsigned';
else
    last_uncram_slice_width = output_bit_width;
    last_uncram_bin_pt = output_bin_pt;
    last_uncram_arith_type = '1';
    convert_arith_type = 'Signed  (2''s comp)';
end

reuse_block(blk, 'last_uncram', 'gavrt_library/uncram',...
    'Position', [x1+3*width y1 x1+4*width y1+height],...
    'Num_Slice', num2str(2^samples_out),...
    'Slice_Width', num2str(last_uncram_slice_width),...
    'Bin_Pt', num2str(last_uncram_bin_pt),...
    'Arith_Type', last_uncram_arith_type);
add_line(blk, 'mux/1', 'last_uncram/1');
reuse_block(blk, 'sync_out', 'built-in/outport',...
    'Position', [x1 y0 x1+width y0+height], 'Port', '1');
add_line(blk, [acc_type, '_acc0/3'], 'sync_out/1');
reuse_block(blk, 'valid', 'built-in/outport',...
    'Position', [x1 y0+60 x1+width y0+height+60], 'Port', '2');
add_line(blk, [acc_type, '_acc0/2'], 'valid/1');
for i=0:2^samples_out-1,
    reuse_block(blk, ['conv', num2str(i)], 'xbsIndex_r4/Convert',...
        'Position', [x1+5*width y1+3*i*height x1+6*width y1+(3*i+1)*height],...
        'Arith_Type', convert_arith_type,...
        'N_bits', num2str(last_uncram_slice_width),...
        'Bin_Pt', num2str(last_uncram_bin_pt),...
        'Quantization', quantization,...
        'Overflow', overflow);
