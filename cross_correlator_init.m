...
for i=0:2^n_inputs-1,
    ...
    if total_lags>n_inputs, 
        start_delay = 2^(total_lags-n_inputs-1);
    else
        start_delay = 0;
    end
...
% Slice off the true lags (without the filler bit)
...
% Split them up appropriately for the stream_vacc
...
% Add the sync input and delay it to align the lags
if strcmp(use_2bit, 'on') && strcmp(complex_in, 'off'),
    mult_sync_delay = 1;
elseif strcmp(use_2bit, 'on') && strcmp(complex_in, 'on'),
    mult_sync_delay = 1 + add_delay;
else
    mult_sync_delay = mult_delay;
end
sync_delay = start_delay + ...
...
reuse_block(blk, 'lag_delay',...
    'xbsIndex_r4/Delay',...
    'Position', [x1-2*width+64 y1-4*height x1-width+64 y1-3*height],...
    'Latency', num2str(sync_delay));
add_line(blk, 'sync/1', 'lag_delay/1');

% Now add the stream_vacc and hook everything up
reuse_block(blk, 'stream_vacc', 'cfa_library/stream_vacc',...
    ...
    'Specify_Add', specify_add,...
    'Add_Spec', add_spec);
add_line(blk, 'lag_delay/1', 'stream_vacc/1');
...
if strcmp(use_2bit, 'off'),
    fmtstr = sprintf('%d lags, Demux: %d, Outputs: %d\n%d_%d=>%d_%d, %s, %s, %s\n Mult: %s, Add: %s',...
        ...
        stream_bit_width, stream_bin_pt, complexity, quantization, overflow,...
        get_var('mult_spec', 'defaults', defaults, varargin{:}),...
        get_var('add_spec', 'defaults', defaults, varargin{:}));
else
    fmtstr = sprintf('%d lags, Demux: %d, Outputs: %d\nSpecial 2-bit %s binary encoding\n Mult: %s, Add: %s',...
        2^total_lags, 2^n_inputs, 2^n_outputs, complexity,...
        get_var('mult_spec', 'defaults', defaults, varargin{:}),...
        get_var('add_spec', 'defaults', defaults, varargin{:}));
end
