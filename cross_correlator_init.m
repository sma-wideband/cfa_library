...
for i=-2^(total_lags-1):2^(total_lags-1)-1,
    ...
    reuse_block(blk, name, 'dali_library/single_lag',...
        ...
        'shift_i', 'on');
    if i==-2^(total_lags-1),
        reuse_block(blk, 'blank_lag', 'xbsBasic_r4/Constant',...
...
reuse_block(blk, 'lag_slice', 'xbsBasic_r4/Slice',...
...
reuse_block(blk, 'uncram', 'gavrt_library/uncram',...
    ...
    'Num_Slice', num2str(2^(total_lags-n_inputs-1)),...
    'Slice_Width', num2str(lagged_bit_width*2^(n_inputs+1)),...
    'Bin_Pt', '0',...
    'Arith_Type', '0');
...
reuse_block(blk, 'stream_vacc', 'dali_library/stream_vacc',...
    ...
    'Samples_Out', num2str(n_inputs+1),...
...
for i=1:2^(total_lags-n_inputs-1),
...
for i=0:2^(n_inputs+1)-1,
    ...
    reuse_block(blk, output_name, 'built-in/outport',...
        'Position', [x1+300 y1+i*height x1+300+width y1+(i+1)*height],...
        'Port',  num2str(2+i));
    add_line(blk, ['stream_vacc/', num2str(i+2)], [output_name, '/1']);
end

% Clean blocks and finish up.
clean_blocks(blk);
fmtstr = sprintf('%d lags, Demux: %d,\n%d_%d=>%d_%d, %s, %s\n Mult: %s, Add: %s',...
    2^total_lags, 2^n_inputs, input_bit_width, input_bin_pt,...
